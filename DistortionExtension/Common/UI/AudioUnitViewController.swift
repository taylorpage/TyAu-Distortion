...
private let log = Logger(subsystem: "com.taylor.audio.DistortionExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<DistortionExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try DistortionExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? DistortionExtensionAudioUnit else {
				log.error("Unable to create DistortionExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(DistortionExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = DistortionExtensionMainView(parameterTree: observableParameterTree)
