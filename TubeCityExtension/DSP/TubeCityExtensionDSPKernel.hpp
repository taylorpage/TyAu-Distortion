...
public:
    ...
    // Initialize pre-tubecity EQ filters
    void initializeEQ(double sampleRate) {
        ...
        // High-pass at 75Hz (removes low rumble before tubecity)
    ...
    // Apply fixed pre-tubecity EQ to shape tone
    float applyPreEQ(float input, int channel) {
        // High-pass filter at 75Hz (removes rumble/mud before tubecity)
    ...
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case TubeCityExtensionParameterAddress::drive:
                mDrive = value;
                break;
    ...
    // Pre-tubecity EQ filter state (per-channel)
